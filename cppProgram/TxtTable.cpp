...
std::string TxtTable::print_table() {
    ...
    for (int i = 0; i < max_col_size; i++) {
        if (col_max_wide[i]) {
            ...
            total_table += reqeat_char(max_len - col_num_to_str(i).length(),' ');
            total_wide += (max_len + 3);
        }
    }
    total_table += "\n";
    for (int i = 0; i < max_row_size; i++) {
        total_table += reqeat_char(total_wide, '-');
        total_table += "\n" + std::to_string(i + 1);
        total_table += reqeat_char(4 - std::to_string(i + 1).length(), ' ');
        ...
        for (int j = 0; j < max_col_size; j++) {
            if(col_max_wide[j]) {
                ...
                total_table += " | " + s;
                total_table += reqeat_char(max_len - s.length(), ' ');
            }
        }
        total_table += "\n";
    }
...
std::string TxtTable::reqeat_char(int n, char c) {
    std::string s = "";
    for (int i = 0; i < n; i++) {
        s.push_back(c);
    }
